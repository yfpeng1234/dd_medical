...
#choose subset from original dataset
choose_idx=np.random.choice(training_data['x'].shape[0],dd_num,replace=False)
subset={'x':training_data['x'][choose_idx],'y':training_data['y'][choose_idx]}

#eval
net=dde.maps.FNN(layer_size, activation, initializer)
model=dde.Model_General(net,pde,subset,training_data,testing_data)
