...
class Model_General:
    ...
    def __init__(self, net,pde,training_data,testing_data,eval_data):
        self.net = net
        self.pde=pde
        ...
        self.train_state.set_data_train(self.training_data['x'],self.training_data['y'])
        self.train_state.set_data_test(self.testing_data['x'],self.testing_data['y'])
    ...
    def unified_loss(self,inputs,targets,outputs,loss_fn):
        ...
        #loss_fn is a two-element list, one for PDE constrain loss, one for x->y mapping loss
        loss_fn = [loss_fn] * 2

        losses = [
            loss_fn[0](bkd.zeros_like(error), error) for i, error in enumerate(f)
        ]

        error=outputs-targets
        losses.append(loss_fn[1](bkd.zeros_like(error), error))
        return losses
    ...
    def _compile_pytorch(self, lr, loss_fn, decay):
        ...
        def outputs_losses(training, inputs, targets, auxiliary_vars, losses_fn):
            ...
            # if forward-mode AD is used, then a forward call needs to be passed
            aux = [self.net] if config.autodiff == "forward" else None
            losses = losses_fn(inputs,targets,outputs_,loss_fn)
            if not isinstance(losses, list):
                losses = [losses]
            losses = torch.stack(losses)
            # Weighted losses
        ...
        def outputs_losses_train(inputs, targets, auxiliary_vars):
            return outputs_losses(
                True, inputs, targets, auxiliary_vars, self.unified_loss
            )

        def outputs_losses_test(inputs, targets, auxiliary_vars):
            return outputs_losses(
                False, inputs, targets, auxiliary_vars, self.unified_loss
            )
    ...
    def _test(self,mode):
        ...
        if mode=='train':
            ...
            self.train_state.set_data_test(self.eval_data['x'],self.eval_data['y'])
        ...
        return self.train_state.metrics_test[0]
