...
def hide_variables(data_list, num_features_to_hide, num_features_per_time_slice = 200, num_time_slices = 2):
    ...
    for i in range(len(data_list)):
        ...
        # choosing randomly the variables to hide and make sure same variable across time slices are hidden
...
def save(data_list):
    for i in range(len(data_list)):
        ...
        if os.path.isdir("./../data/seperated_data_10") == False:
            os.mkdir("./../data/seperated_data_10")
        path = "./../data/seperated_data_10/partition_" + str(i) + ".csv"
        new.to_csv(path, index=False)

# reading data 
data_R_dbn = pd.read_csv("./../data/original_train_10.csv")
